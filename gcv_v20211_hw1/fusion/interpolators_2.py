...
def pairwise_interpolate_predictions(
    ...
    for idx, point_from_j in tqdm(enumerate(reprojected_j)):
        ...
        point_from_j_nns = np.concatenate((uv_i[point_nn_indexes], image_i_flattened[point_nn_indexes].reshape(-1, 1)), axis=1)
        ...
        if interp_mask[idx]:
            ...
            try:
                ...
                tck = interpolate.bisplrep(point_from_j_nns[:, 0], point_from_j_nns[:, 1],
                                           distances_i.flatten()[point_nn_indexes], kx=1, ky=1)
